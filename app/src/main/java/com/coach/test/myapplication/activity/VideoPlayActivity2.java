...
//import com.coach.test.coachmw.manager.RawManager;
 ...
 *
 */
public class VideoPlayActivity2 extends WActivity implements SurfaceHolder.Callback {
		...
		mIBluetooth = new IBluetooth() {
			...
			public void connectionState(int state) {
				if(state == BluetoothManager.DISCONNECT) { // 연결됨.
					runOnUiThread(new Runnable() {
						public void run() {
							SoundPoolManager.playBeep(VideoPlayActivity2.this);
		...
		*/
        ...
        mPlayer.setOnCompletionListener(new OnCompletionListener() {
            ...
            public void onCompletion(MediaPlayer mp) {
                /** 동영상이 끝나면 수행 **/
                SoundPoolManager.playEnd(VideoPlayActivity2.this);
                stop();
                finish();
            }
        });
